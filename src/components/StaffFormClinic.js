...
export const StaffFormClinic = ({
  ...
  const [dob, setDob] = useState(personalData.dob);
  const [postalCode, setPostalCode] = useState(personalData.postalCode);
  const [phoneNumber, setPhoneNumber] = useState(personalData.phoneNumber);
  const [workEmail, setWorkEmail] = useState(personalData.workEmail);
  const [isConsent, setIsConsent] = useState(false);

  const validateMessage = () => {
    if (hasValidate && isValid) {
      return (
        <div>
